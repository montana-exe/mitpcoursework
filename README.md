...
3. **OpenCL выделен отдельным контуром.** Класс `OpenCLEvaluator` и kernel `cpp/kernels/fitness.cl` выполняют пакетную оценку расстояний популяции. Если OpenCL недоступен, система остаётся работоспособной на CPU; это важно для CI и обычных учебных машин.
