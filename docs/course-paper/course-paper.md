...
В рамках курсовой работы используется упрощённая, но инженерно полезная модель. Каждый клиент имеет координаты и величину спроса. Депо задаёт начало и конец каждого рейса. Автомобиль описывается грузоподъёмностью и максимальной длительностью маршрута. Расстояние между точками рассчитывается как евклидово, что позволяет сосредоточиться на алгоритмической части. При дальнейшем развитии проекта этот слой можно заменить дорожной матрицей расстояний, полученной из картографического сервиса или внутренней геоинформационной системы.
...
flowchart LR
    ...
    Solver --> OpenCL[OpenCL distance evaluator]
...
Класс `OpenCLEvaluator` отвечает за пакетную оценку расстояний хромосом. Он изолирует работу с OpenCL context, device, command queue, buffers и kernel. Если OpenCL-платформа отсутствует, C++-ядро сохраняет работоспособность на CPU. Наличие fallback не отменяет GPU-контур: при доступной OpenCL-платформе расстояния популяции рассчитываются через kernel `route_distance_kernel`.

### 2.4 Использование OpenCL для GPU-ускорения

Оценка популяции является одной из наиболее удобных частей генетического алгоритма для параллельного исполнения. Каждая хромосома может оцениваться независимо от остальных, поэтому пакет особей естественно отображается на сетку OpenCL work-items. В проекте каждый work-item вычисляет длину цикла для одной хромосомы. CPU-код после этого добавляет штрафы за ограничения, поскольку грузоподъёмность и разбиение на рейсы зависят от пошаговой процедуры декодирования.

Такое разделение выбрано сознательно. Полный перенос всей логики декодирования на GPU усложнил бы kernel и ухудшил читаемость учебного проекта. Расчёт расстояний, напротив, имеет регулярную структуру и хорошо подходит для OpenCL. При дальнейшем развитии можно перенести на GPU и часть расчёта штрафов, но уже с отдельным представлением маршрутов и префиксными суммами спроса.
...
Python-интерфейс состоит из трёх частей. Первая часть — модели данных `Customer`, `Vehicle`, `SolverSettings`, `OptimizationRequest` и `OptimizationResponse`. Они описывают контракт API и предотвращают попадание некорректных данных в C++-ядро. Вторая часть — класс `NativeRouteOptimizer`, который ищет нативную библиотеку, настраивает сигнатуры `ctypes` и выполняет преобразование данных. Третья часть — FastAPI-приложение с маршрутами `/health` и `/optimize`.

Визуализация вынесена в отдельный модуль `visualization.py`. Он не участвует в оптимизации и может использоваться независимо в экспериментах, отчётах или демонстрационных сценариях. Такое разделение снижает связанность: API не зависит от графического вывода, а визуализация работает с теми же моделями данных, что и основной сервис.
...
| `Problem` | `nodes`, `depot_index`, `vehicle_capacity`, `max_route_time` | Полное описание задачи |
...
sequenceDiagram
    ...
    C->>G: solve(problem)
    G->>O: evaluate_cycle_distances(population)
    O-->>G: distance vector
    G-->>C: Solution
...
Функция приспособленности включает расстояние и штрафы. Расстояние рассчитывается по маршрутам, которые получаются при декодировании хромосомы. Если выбран OpenCL backend и платформа доступна, расстояние цикла для особей оценивается пакетно на OpenCL-устройстве, а CPU добавляет штрафы за ограничения. Штраф применяется при превышении грузоподъёмности, нарушении максимального времени маршрута или некорректной вершине.
...
OpenCL применяется для ускорения массовой оценки расстояний. Генетический алгоритм обрабатывает популяцию, а каждая особь может оцениваться независимо. Такая структура хорошо соответствует модели OpenCL: host-код подготавливает буферы, kernel выполняется параллельно для множества work-items, результат возвращается в host-память.
...
python/routeopt/          Python API, модели, ctypes-интеграция, визуализация
...
GPU-контур представлен классом `OpenCLEvaluator` и kernel `route_distance_kernel`. Host-код выбирает OpenCL-платформу и устройство, создаёт context, command queue, program, kernel и буферы. В буферы передаются координаты точек и плоский массив хромосом. Каждый work-item рассчитывает длину цикла одной особи.
...
В текущей архитектуре OpenCL ускоряет регулярную часть оценки, а CPU сохраняет контроль над ограничениями маршрута. Это уменьшает сложность kernel и оставляет проверяемую логику ограничений в C++-коде. При наличии OpenCL runtime solver сообщает backend `opencl-distance-cpu-constraints`; при отсутствии runtime используется CPU и backend `cpu-opencl-unavailable`.

### 4.4 Реализация Python API

Python API реализован в `python/routeopt/api.py`. Приложение содержит endpoint `/health` для проверки состояния и `/optimize` для запуска оптимизации. Модели Pydantic задают контракт запроса и ответа. Проверка уникальности идентификаторов клиентов вынесена в `model_validator`, чтобы ошибка обнаруживалась до передачи данных в нативный слой.
...
Нативная библиотека обнаруживается через переменную окружения `ROUTEOPT_LIB` или стандартные пути сборки. Если библиотека отсутствует, используется Python fallback, чтобы API и тесты можно было запускать до компиляции C++-части. В отчётах производительности такой режим не используется как основной.

### 4.5 Визуализация маршрутов

Модуль `visualization.py` строит изображение маршрутов через Matplotlib. Депо отображается отдельным маркером, клиенты — точками, маршруты — линиями, возвращающимися в депо. Заголовок графика содержит суммарное расстояние и число маршрутов. Визуализация полезна для проверки качества: даже при корректной числовой метрике на графике быстро видны пересечения, удалённые точки и неудачное разбиение на рейсы.

Визуализация отделена от API и C++-ядра. Такое решение позволяет запускать оптимизацию на сервере без графической подсистемы, а изображения строить только в экспериментальных скриптах или при подготовке отчёта.
...
```json
{
  ...
  "vehicle": {"capacity": 10, "max_route_time": 220},
...
```json
{
  ...
  "feasible": true,
  "backend": "native-opencl-distance-cpu-constraints"
}
```
...
В ходе работы разработана система оптимизации маршрутов доставки на основе генетического алгоритма. Реализованы C++-ядро, C API, Python-интерфейс, FastAPI-сервис, визуализация, OpenCL-контур для пакетной оценки расстояний, тесты, бенчмарки, Docker-конфигурация и CI/CD pipeline.
...
Для подготовки скриншотов необходимо запустить API через Docker Compose, открыть Swagger UI по адресу `http://localhost:8000/docs`, выполнить запрос `/optimize` и сохранить экран с ответом. Для карты маршрутов следует вызвать `render_routes` и сохранить PNG-файл, полученный из экспериментального сценария.
